function [f, df, hessian] = ln_poisson_model(param,data,modelType,numPos,numHD,numSpd,numTheta)
...
% find the parameters
% numPos = 450; numHD = 18; numSpd = 10; numTheta = 18; % hardcoded: number of parameters
...
function [J,J_g,J_h] = rough_penalty_2d(param,beta)
    ...
    msize = ceil(sqrt(numParam));
    D1 = spdiags(ones(msize,1)*[-1 1],0:1, msize-1, msize);
    DD1 = D1'*D1;
    M1 = kron(eye(msize),DD1); M2 = kron(DD1,eye(msize));
    M = (M1 + M2);
    
    M = M(1:numParam,1:numParam);
...
%% for 7 models
% function [param_pos,param_hd,param_spd] = find_param(param,modelType,numPos,numHD,numSpd)
% 
% param_pos = []; param_hd = []; param_spd = [];
% 
