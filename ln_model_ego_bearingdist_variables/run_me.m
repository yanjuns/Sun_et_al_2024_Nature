...
for n = 1:length(datapath)
    ...
    if strcmp(p.n2compute,'all')
        n2compute = repmat({1:length(thresh)},1,length(neuronIndiv));
    else
        load('corner_metricsR1.mat','C')
        n2compute = C.cornercell(idx);
    end
    t0 = tic();
...
%             end
    ...
    save(['lnp_',p.dataType,'_',method,'ctrdist.mat'], 'lnp_model', 'llh', 'fit_results','p','-v7.3')
end
