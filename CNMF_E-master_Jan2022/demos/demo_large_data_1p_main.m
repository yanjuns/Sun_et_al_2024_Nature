...
%% choose data
neuron = Sources2D();
% nam = get_fullname('/Users/suoqinjin/Documents/CNMF_E-master-1-11-2019/demos/data_1p.tif');          % this demo data is very small, here we just use it as an example
nam = [];
nam = neuron.select_data(nam);  %if nam is [], then select data interactively

%% parameters
% -------------------------    COMPUTATION    -------------------------  %
pars_envs = struct('memory_size_to_use', 700, ...   % GB, memory space you allow to use in MATLAB
                   'memory_size_per_patch', 175, ...   % GB, space for loading data within one patch
                   'patch_dims', [128, 128]);  %pixles, patch size
...
deconv_options = struct('type', 'ar1', ... % model of the calcium traces. {'ar1', 'ar2'}
    ...
    'optimize_smin', true, ...         % minimum spike size. When the value is negative, the actual threshold is abs(smin)*noise level
    'optimize_pars', true, ...  % optimize AR coefficients
...
ring_radius = 18;  % when the ring model used, it is the radius of the ring used in the background model. orinal 18
...
merge_thr_spatial = [0.8, 0.5, -inf];  % merge components with highly correlated spatial shapes (corr=0.8) and small temporal correlations (corr=0.1)

% -------------------------  INITIALIZATION   -------------------------  %
K = [];             % maximum number of neurons per patch. when K=[], take as many as possible.
min_corr = 0.93;     % minimum local correlation for a seeding pixel % use 0.93 for Ai94 mouse, 0.9 for Thy1-G mouse, if ssub=1
min_pnr = 12;       % minimum peak-to-noise ratio for a seeding pixel; defalut 8
...
delete(gcp('nocreate')); % try to solve the problem of aborted works
gcp;
...
[Coor,centroid] = neuron.show_contours(0.8,[],[],1);
colormap winter;
title('Contours of estimated neurons','FontSize',12);
savefig(gcf, 'Contour1.fig');

Coor = neuron.show_contours(0.8);
colormap winter;
title('Contours of estimated neurons','FontSize',12);
savefig(gcf, 'Contour2.fig');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Suoqin%%%%%%%%%%%%%%%%%%%%%%%%%modifications%%%%%%%%%%%%%%%%%%%%%%%%
%% compute some useful variables for further analysis (Jan-2019,S.J.)
neuron.numFrame = neuron.P.numFrames;
neuron.num2read = neuron.frame_range(2);
neuron.centroid = centroid;
sd = GetSn(neuron.C_raw);
neuron.sd = sd;
neuron.trace_raw = neuron.C_raw./repmat(sd,1,size(neuron.C_raw,2));
neuron.trace = neuron.C./repmat(sd,1,size(neuron.C,2));
neuron.A_prev = [];
neuron.C_prev = [];

%% save neuron valiable
save ('M4111h_Neuron.mat', 'neuron', '-v7.3', '-nocompression');
