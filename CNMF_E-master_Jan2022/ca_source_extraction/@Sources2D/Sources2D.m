...
%         info;
        ...
        ab; % allocentric bearing (to nearest corner)
        eb; % egocentric bearing (to nearest corner)
        edist; % egocentric distance (to nearest corner)
        ctrb; % egocentric bearing (to environment center)
        ctrdist; % egocentric distance (to environment center)
        wallb; % egocentric bearing (to nearest wall)
        walldist; % egocentric distance (to nearest wall)
    end
    ...
    methods
        ...
        %% save results
        function file_path = save_workspace(obj, zip_file_path, original_log_folder, original_logfile)
        ...
        function [Coor,cm] = show_contours(obj, thr, ind, img, with_label)
            ...
            init_fig;
            [~,~,cm] = plot_contours(obj.A(:, ind), img, thr,with_label, [], obj.Coor(ind), 2);
            colormap gray;
            try
