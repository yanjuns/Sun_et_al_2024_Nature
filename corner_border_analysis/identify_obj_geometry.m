function [S,N,env_coor,env_coori] = identify_obj_geometry(datapath)
...
for ii = 1:length(S)
    ...
    switch ss
        case{'circle'}
            numcor = 0; %number of corners
        case{'square','squarelw','rect','largesq','largeRT',...
                'largeRTd0','largeRTd90',...
                'trapezoid','squareNoLight','squareNoSense','largeXm1d0',...
                'largeXm1d90','squarebsl','oval','ovald90','largeXbsl'}
            numcor = 0;
...
for ii = 1:length(sessions)
    if N(ii) > 0
        ...
        BW = BW > 0;
        stats = regionprops(BW, {'Centroid','Extrema'});
        env_coor{ii} = [stats.Centroid;stats.Centroid];
    end
end
