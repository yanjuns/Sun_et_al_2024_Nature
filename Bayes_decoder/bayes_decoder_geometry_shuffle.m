function decodeResults = bayes_decoder_geometry_shuffle(datapath,condition)
...
for k = 1:numFolds
    ...
    parfor iter = 1:1000
        % shuffle dataset
        idx = randperm(length(data.position_ds));
        position_shuff = data.position_ds(idx');

        % training dataset
        neuron_train = data.neuron_ds(train_ind,:);
        behav_train = position_shuff(train_ind,:);
        ...
        accuracy(iter) = numel(find(testdata.position_ds == label))/numel(label);
    end
    decodeResults{k} = accuracy; 
end
