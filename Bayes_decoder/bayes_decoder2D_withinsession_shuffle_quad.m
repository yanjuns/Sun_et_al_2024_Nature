function [decodeShuffle,correctquadShuffle] = bayes_decoder2D_withinsession_shuffle_quad(neuronIndividualsf, behavIndividualsf, ...
    thresh, mquad, placecell, nboot, numFolds, frame_avg, binsize, binWidth)
...
correctquadShuffle = cell(numFolds, length(neuronIndividualsf));
for n = 1:length(neuronIndividualsf)
    ...
    for k = 1:numFolds
        ...
        parfor iter = 1:nboot
            ...
            % calculate decoding quad accuracy
            truequad = diag(mquad_ea(positionBin_ds(:,2),positionBin_ds(:,1)));
            decodedquad = diag(mquad_ea(decodePosition(:,2),decodePosition(:,1)));
            correctquad = truequad == decodedquad;
            correctquad = sum(correctquad)/numel(correctquad);
            correctquadAll = [correctquadAll;correctquad]
        end
        
        %% Output final data
        decodeShuffle{k,n} = dEshuffle;
        correctquadShuffle{k,n} = correctquadAll;
    end
end
